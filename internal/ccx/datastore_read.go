...
func getPortFromDatastore(c Datastore) (int, error) {
	var port int
...
func (svc *DatastoresClient) Read(ctx context.Context, id string) (*Datastore, error) {
	...
	c := Datastore{
		...
		VolumeType:          StringVal(rs.DiskType),
		VolumeSize:          Uint64Val(rs.DiskSize),
		VolumeIOPS:          Uint64Val(rs.InstanceIOPS),
