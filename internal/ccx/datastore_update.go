...
func (svc *DatastoresClient) Update(ctx context.Context, old, next Datastore) (*Datastore, error) {
	out := &old
...
func (svc *DatastoresClient) update(ctx context.Context, old, next Datastore) (bool, error) {
...
func (svc *DatastoresClient) resize(ctx context.Context, old, next Datastore) (bool, error) {
	...
	if adding {
		...
		if next.VpcUUID == "" && missing > 0 { // allocate AZs if public and need is less than have
			...
			if err != nil {
				return false, fmt.Errorf("%w: %w", ErrAllocatingAZs, err)
			}
...
func (svc *DatastoresClient) updateSizeRequest(ctx context.Context, old, next Datastore) (updateRequest, error) {
	...
	if old.VpcUUID == "" && missing > 0 { // allocate AZs if public and need is less than have
		allAzs, err := svc.contentSvc.AvailabilityZones(ctx, next.CloudProvider, next.CloudRegion)
		if err != nil {
			return ur, fmt.Errorf("%w: %w", ErrAllocatingAZs, err)
		}
...
func (svc *DatastoresClient) updateRequest(old, next Datastore) (updateRequest, bool) {
	var (
		ur            updateRequest
		ok            bool
		cv            changeVolume
...
func oldestRemovableNodeIds(hosts []Host, count int) ([]string, error) {
	hosts = slices.DeleteFunc(hosts, func(h Host) bool {
		return h.IsPrimary()
	})
...
func newestNodeSpecs(hosts []Host, count int, azs []string, instanceSize string) ([]hostSpecs, error) {
	if len(hosts) == 0 {
