...
type HTTPClient interface {
	...
	// errors returned are:
	// - RequestEncodingErr (if body encoding fails)
	// - RequestInitializationErr (if request creation fails)
	// - RequestSendingErr (if request sending fails)
	// - ResourceNotFoundErr (if API returns 404)
	// - ApiErr (if API returns 4xx or 5xx)
...
func DecodeJsonInto(body io.ReadCloser, target any) error {
	...
	if err != nil {
		return errors.Join(ErrResponseReadFailed, err)
	}

	err = json.Unmarshal(raw, target)
	if err != nil {
		return errors.Join(ErrResponseDecoding, err)
	}
...
// errors returned are:
// - ErrRequestEncoding (if body encoding fails)
// - ErrRequestInitialization (if request creation fails)
// - ErrRequestSending (if request sending fails)
// - ErrResourceNotFound (if API returns 404)
// - ErrApi (if API returns 4xx or 5xx)
func (h *basicHTTPClient) Do(ctx context.Context, method, path string, body any) (*http.Response, error) {
	var b bytes.Buffer
	if body != nil {
		if err := json.NewEncoder(&b).Encode(body); err != nil {
			return nil, errors.Join(ErrRequestEncoding, err)
		}
	}

	req, err := http.NewRequestWithContext(ctx, method, h.baseURL+path, &b)
	if err != nil {
		return nil, errors.Join(ErrRequestInitialization, err)
	}

	rs, err := h.cli.Do(req)

	if err != nil {
		return nil, errors.Join(ErrRequestSending, err)
	} else if rs.StatusCode == http.StatusNotFound {
		return nil, ErrResourceNotFound
	} else if rs.StatusCode >= http.StatusBadRequest {
		return nil, fmt.Errorf("%w: %w", ErrApi, ErrorFromResponse(rs))
	}
...
// - ErrResourceNotFound (if API returns 404)
// - ErrApi (if API returns 4xx or 5xx)
func (h *basicHTTPClient) Get(ctx context.Context, path string, target any) error {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, h.baseURL+path, nil)
	if err != nil {
		return errors.Join(ErrRequestInitialization, err)
	}

	rs, err := h.cli.Do(req)

	if err != nil {
		return errors.Join(ErrRequestSending, err)
	} else if rs.StatusCode == http.StatusNotFound {
		return ErrResourceNotFound
	} else if rs.StatusCode >= http.StatusBadRequest {
		return fmt.Errorf("%w: %w", ErrApi, ErrorFromResponse(rs))
	}
